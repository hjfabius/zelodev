...
extern 	@near 			BYTE 		m_arbyScanningHeadSignal[(ENCODER_PULSES/4)];
...
BYTE windowOscilloscopeGetValue(WORD a_woEncIndex)
{
	...
	a_woEncIndex = (a_woEncIndex+8)& (ENCODER_PULSES-1);
...
WORD windowOffsetToHaveCenteredGate(void)
{
	...
	if ((m_woEncoderGateStart != 0xFFFF) && (m_woEncoderGateEnd != 0xFFFF))
	{
		...
		{
			woEncoderGateCenter = ((ENCODER_PULSES+m_woEncoderGateEnd+m_woEncoderGateStart)/2)& (ENCODER_PULSES-1);
		}
		
		return (WORD) (ENCODER_PULSES + woEncoderGateCenter - (((WINDOW_3_WIDTH-4)/2)*parametersGetValue(PARAM_OSCIL_ZOOM)))& (ENCODER_PULSES-1);
	}
	return 0x0000;
...
/*	if (parametersIsChanged(PARAM_CORRECTION))
	...
	if ((m_woEncoderGateStart != 0xFFFF) && (m_woEncoderGateEnd != 0xFFFF))
	{
		...
		{
			woEncoderIndexOnGate = (ENCODER_PULSES + woEncoderIndex - parametersGetValue(PARAM_OSCIL_MANUAL_OFFSET)) & (ENCODER_PULSES-1);
		}
		else
		{
		...
		byPrevGateValue = windowIsInGate((woEncoderIndexOnGate+parametersGetValue(PARAM_OSCIL_ZOOM)-1)& (ENCODER_PULSES-1))?WINDOW_3_GATE_LOW_Y:WINDOW_3_GATE_HIGH_Y;
		...
		for(i=0; (i<(WINDOW_3_WIDTH-4)&&(!displayIsKeyPressed())) ; i++)
		{
			...
			while(j-->0)
			{
				...
				woEncoderIndexOnGate = (woEncoderIndexOnGate+1) & (ENCODER_PULSES-1);
			}
			...
			))  // Is not only a refresh
	{
		...
		switch(*a_byGroup)
		{
			...
			case MENU_GROUP_PARAMETER:
				...
				if (
					(parametersGetValue(PARAM_DIVISOR_ENABLE) == 0x0000)
				   )
				{
					displayWriteString(labelGet(LABEL_DIVISOR_01),		DISPLAY_NULL, 				78, 	52);
				}
