...
//		IC1CONbits.ICM = 0b010; 				// Generate capture event on every falling S1 edge (Timer T2 40Mhz)
//		IC2CONbits.ICM = 0b010; 				// Generate capture event on every falling S1 edge (Encoder A)
		IC1CONbits.ICM = 0b011; 				// Generate capture event on every rising S1 edge (Timer T2 40Mhz)
		IC2CONbits.ICM = 0b011; 				// Generate capture event on every rising S1 edge (Encoder A)
		PIN_GATE = !PIN_GATE;
	}	
	*/
	...
	else
	{
		...
		else if(m_woEncoderPosition == m_arwoGateStart[PULSE_QEI][1])  
		{
		...
		else if (m_woEncoderPosition == m_arwoGateEnd[PULSE_QEI][1]) 
		{
			m_ucEncoderArea = ENCODER_AREA_OUT_OF_GATE;
			PIN_GATE = 0;
			IC1CONbits.ICM = 0; 	// Disable IC1
