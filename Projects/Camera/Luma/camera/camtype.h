/* Camera Definitions 
   ...
   $Log: camtype.h,v $
   Revision 1.11  2002/10/21 11:41:02  tf
   New videomode VM_DIG14

   Revision 1.10  2002/06/18 15:03:53  tf
   New videomodes: VM_DIG2x12,VM_DIG2x14, VM_DIG2x16

   Revision 1.9  2002/06/18 07:07:26  tf
   VM_G4 removed.

   Revision 1.8  2002/04/11 09:53:41  rs
   ...
   Revision 1.6  2002/02/19 16:06:52  tf
   New Videomodes VM_DIG2x10 und VM_G4.

   Revision 1.5  2001/08/08 12:37:01  rs
   Incorrect char* type:
   -> char FAR* changed to const char FAR* if possible

   Revision 1.4  2001/08/08 11:01:44  rs
   Merged with version 1.2 from the cvs-module 'Driver'

   Revision 1.3  2001/06/11 09:35:24  hs
   no message

   Revision 1.2  2001/03/22 10:12:12  hs
...
typedef struct camtype_t
{
    ...
    WORD VideoMode;              /*                                                */
    WORD Interlaced;             /* number of fields per frame                     */
...
#define CAM_VIDEO_MODES   {"VM_AUTO",   "VM_CCIR",      "VM_RS170",     "VM_PAL",       "VM_NTSC",\
                           ...
                           "VM_TVES",   "VM_NTSC_SVHS", "VM_SECAM_SVHS","VM_DIG2X8",    "VM_DIG2X10", \
                           "VM_DIG2X12","VM_DIG2X14",   "VM_DIG2X16",   "VM_DIG14", \
                           NULL}
...
#define _GetHPix(p,v) (((p)->HTParameterMode  != 1) ? (v) : _TimeToPix(p,v) )
#define _GetHTime(p,v) (((p)->HTParameterMode == 1) ? (v) : _PixToTime(p,v) )

#define _GetVLine(p,v) (((p)->VTParameterMode != 1) ? (v) : _TimeToLine(p,v) )
#define _GetVTime(p,v) (((p)->VTParameterMode == 1) ? (v) : _LineToTime(p,v) )
...
extern "C" {
...
#if defined(linux) || defined(WIN32) || defined(__WIN32__) || defined(__386__) || defined(__DPMI32__)
 ...
 #ifdef __BORLANDC__
  #ifdef CAMDEF_DLL
   #define CAMDEFAPI(fkttype)  fkttype _export _stdcall
  #else
...
#endif
#elif defined(linux)
#define CAMDEFAPI(fkttype)  extern fkttype
#else
#define CAMDEFAPI(fkttype)  fkttype
#endif
...
/* use MaxCameraDefs=0 if the real count of camera definitions is unknown */

CAMDEFAPI(VOID) mvFreeCamTypeDefinitions  ( VOID );

/* use together with dyncamic driver dll loading: */
CAMDEFAPI(WORD) mvInitCamTypeDefDynamic  ( const char* dllname, WORD MaxCameraDefs );
CAMDEFAPI(VOID) mvFreeCamTypeDefDynamic  ( VOID );


WORD _MVDFKT INTERPRETER_FKT mvSelCamera               ( DEV_T FAR* dev, const char FAR* Name);
void _MVDFKT INTERPRETER_FKT mvDefCamType              ( DEV_T FAR* dev, const char FAR* Name, const char FAR* VideoMode, const char FAR* Interlaced, WORD VFreq, WORD HFreq, WORD ClockFreq, const char FAR* ClockMode );
void _MVDFKT INTERPRETER_FKT mvDefCamAcquireSetup      ( DEV_T FAR* dev, const char FAR* Name, const char FAR* AcquireMode, const char FAR* Invert, const char FAR* StartMode );
void _MVDFKT INTERPRETER_FKT mvDefCamGenerateVSync     ( DEV_T FAR* dev, const char FAR* Name, DWORD Start, DWORD Width );
void _MVDFKT INTERPRETER_FKT mvDefCamAnalogParam       ( DEV_T FAR* dev, const char FAR* Name, const char FAR* ChromFilter, WORD Gain, INT OffsetVol, WORD LowRefVol, WORD HighRefVol);
WORD _MVDFKT INTERPRETER_FKT mvDefHorizontalUnit       ( DEV_T FAR* dev, const char FAR* Name, const char FAR* Unit );
WORD _MVDFKT INTERPRETER_FKT mvDefVerticalUnit         ( DEV_T FAR* dev, const char FAR* Name, const char FAR* Unit );

VOID _MVDFKT INTERPRETER_FKT mvDefCamHorizontalAcquire ( DEV_T FAR* dev, const char FAR* Name, DWORD Start, DWORD Width, WORD Increment );
VOID _MVDFKT INTERPRETER_FKT mvDefCamVerticalAcquire   ( DEV_T FAR* dev, const char FAR* Name, DWORD Start, DWORD Width, WORD Increment );
VOID _MVDFKT INTERPRETER_FKT mvDefCamClamp             ( DEV_T FAR* dev, const char FAR* Name, DWORD Start, DWORD Width );
VOID _MVDFKT INTERPRETER_FKT mvDefCamZero              ( DEV_T FAR* dev, const char FAR* Name, DWORD Start, DWORD Width );
VOID _MVDFKT INTERPRETER_FKT mvDefCamFieldGate         ( DEV_T FAR* dev, const char FAR* Name, DWORD Start, DWORD Width );
VOID _MVDFKT INTERPRETER_FKT mvDefCamGenerateVSync     ( DEV_T FAR* dev, const char FAR* Name, DWORD Start, DWORD Width );

VOID _MVDFKT INTERPRETER_FKT mvDefChannelGain          ( DEV_T FAR* dev, const char FAR* Name, WORD Channel, WORD Gain );
VOID _MVDFKT INTERPRETER_FKT mvDefChannelOffset        ( DEV_T FAR* dev, const char FAR* Name, WORD Channel, INT OffsetVol );
VOID _MVDFKT INTERPRETER_FKT mvDefChannelReference     ( DEV_T FAR* dev, const char FAR* Name, WORD Channel, WORD ReferenceNr, WORD RefVol );
